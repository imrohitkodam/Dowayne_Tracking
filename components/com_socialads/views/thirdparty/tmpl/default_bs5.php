			...
			<div class="tab-pane container active" id="profile">
				...
				<div class="form-group row">
					...
					<div class="col-lg-10 col-md-10 col-sm-9 col-xs-12">
						<p><label>Add the Area(s) that cover your reach</label>
						</p>
						
						<div class="btn-group" role="group" aria-label="Basic example">
						  <button class="btn btn-primary" id="newCampaignCircle" onclick="startDrawingCircle()"><?php echo Text::_('COM_SOCIALADS_THIRD_PARTY_ADD_SERVICE_AREA'); ?></button>
						 	<!-- Button trigger modal -->
						<button  class="btn btn-primary" id="addTextModalOpenButton" data-bs-toggle="modal" data-bs-target="#addTextModal">
							Add Pins From Text 
						</button>

						<button  class="btn btn-primary" id="addCSVModalOpenButton" data-bs-toggle="modal" data-bs-target="#addCSVModal">
							Add Pins From CSV 
						</button>

						<button  class="btn btn-danger" id="clearCircles" onclick="clearCircle()">Clear service area</button>
...
<script type="text/javascript">
	...
	let alreadyPresentCirCles = [];  // If already added services area present then this stores in this js variable

	<?php
	// Check the already added service area present, Also if exist then store value in js array variable
	if (isset($this->locationDetails) && count($this->locationDetails))
	{
		?>
		alreadyPresentCirCles = <?php echo (json_encode($this->locationDetails)); ?>;
		<?php
	}?>
	...
	function adjustViewport() {
		...
		if (markers.length > 1) {
			...
			map.setCenter(markers[0].position); // Center on the single marker
			map.setZoom(10); // Adjust zoom to your preference for a single marker
		}
	}
	...
	techjoomla.jQuery(document).ready(function() {
		...
		jQuery("#addCSVModal").on('hide.bs.modal', function(){
			jQuery('#addCSVModal #csvFormFile').val(null);
		});
		...
		jQuery("#addTextModal #addPinsFromTextSubmit").click(function() {
			...
			jQuery('#addTextModal #addPinsTextArea').val().split(/\r?\n/).forEach(address => {
				if (address.trim() !== "") 
				{
					geocodeAddress(geocoder, address.trim());
				}
			});
			...
			jQuery('#addTextModal .modal-header .btn-close').click();
		...
		jQuery("#addCSVModal #addPinsFromCSVSubmit").click(function() {
			...
			jQuery('#addCSVModal .modal-header .btn-close').click();
