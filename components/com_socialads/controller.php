...
class SocialadsController extends BaseController
{
	...
	public function archiveStats()
	{
		...
		while ($totalRows > 0)
		{
			...
			// Query to get stats
			$query = $db->getQuery(true);
			$query->select('id, ad_id, display_type, time, third_party_id');
			$query->from($db->quoteName('#__ad_stats'));
			...
			foreach ($dbStats as $stat)
			{
				...
				$thirdPartyId = $stat->third_party_id;

				// 0 = imprs; 1 = clks;
				if ($stat->display_type == '0')
				{
					if (isset($newStats[$thirdPartyId][$date][$stat->ad_id]['imprs']))
					{
						$newStats[$thirdPartyId][$date][$stat->ad_id]['imprs'] += 1;
					}
					else 
					{
						$newStats[$thirdPartyId][$date][$stat->ad_id]['imprs'] = 1;
				...
				{
					if (isset($newStats[$thirdPartyId][$date][$stat->ad_id]['clks']))
					{
						$newStats[$thirdPartyId][$date][$stat->ad_id]['clks'] += 1;
					}
					else 
					{
						$newStats[$thirdPartyId][$date][$stat->ad_id]['clks'] = 1;
			...
			foreach ($newStats as $thirdParty => $dateStats)
			{
					...
					foreach ($stats as $id => $data)
					{
						...
						$query->where(
							array(
								...
								$db->quoteName('third_party_id') . ' = ' . (int) $thirdParty
							)
						);
						...
						else
						{
							...
							$stat->click      = $click;
							$stat->third_party_id      = $thirdParty;
