...
class SocialadsControllerAdform extends FormController
{
	...
	public function autoSave()
	{
		...
		$formData  = $input->getArray(
			...
			'selectlocationonmap'  => 'integer',
			'map_targett_id'  => 'integer',
			'map'  => array (
				'population' => 'int',
				'details' => 'text'
			),
		...
		if ($stepId == 'ad-design')
		{
            ...
            $pricingOption = $params->get('pricing_options');

            if(!empty($pricingOption))
            {
                if($pricingOption[0] == "perclick")
                {
                    $formData['pricing_opt'] = 1;
                }
                else if($pricingOption[0] == "perimpression")
                {
                    $formData['pricing_opt'] = 0;
                }
                else if($pricingOption[0] == "chargetogether")
                {
                    $formData['pricing_opt'] = 4;
                }
            }
		...
		if ($stepId == 'ad-targeting')
		{
			...
			$ad_id         = $session->get('ad_id');
			$returndata['mapTargetData'] = $model->getData_map_target($ad_id);
		...
		if ($stepId == 'ad-pricing' && $sa_params->get('payment_mode') == 'wallet_mode')
		{
			...
			$tmplData->mapTargetData = $model->getData_map_target($ad_id);
	...
	private function getAdsInfoOnPayment($formData, $adPreviewData = null)
	{
		...
		elseif($adPreviewData && $adPreviewData->ad_payment_type == 4)
		{
			$cost       = $formData['pric_click'] + $formData['pric_imp'];
		}
