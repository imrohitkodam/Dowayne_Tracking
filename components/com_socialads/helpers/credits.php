...
class SaCreditsHelper
{
	 ...
	 * @since  1.0
	 **/
	public function reduceCredits($adid, $caltype, $ad_charge, $widget = "", $thirdPartyId = 0)
	{
		/**
		...
		if ($saParams->get('payment_mode') == 'wallet_mode')
		{
			if ($ipresult < 1)
			{
				if ($creator != $userid)
				{
					...
					{
						$saStatsHelper->putStats($adid, $caltype, $ad_charge, $widget, $thirdPartyId);
	...
	public static function mailLowBal($adid, $mode)
	{
		...
		$subject = str_replace('[ADTITLE]', $ad_title, $subject);
	...
	public static function checkBalance($adid, $bidPrice)
	{
		...
		$query = "SELECT a.created_by, a.camp_id, c.daily_budget, c.total_budget
		 FROM #__ad_data as a
		...
		if (!empty($info))
		{
			...
			// Prevent ad display if wallet balance is zero or less
			if ($remainingAmt <= 0)
			{
				return 0;
			}
			...
			$query3 = $db->getQuery(true);
			$query3 = "SELECT SUM(spent)
			 FROM `#__ad_wallet_transc`
			 where type_id = " . $camp_id . "
			 AND type='C'";
			$db->setQuery($query3);
			$totalSpent = $db->loadresult();

			if ( (($bidPrice - $remainingAmt) <= 0) && (((int)($spent + $bidPrice) - $dailyBudget) <= 0) && ((((int)($totalSpent + $bidPrice) - $totalBudget) <= 0) || $totalBudget == -1) )
			{
				$status = 1;
			}
		}
