...
abstract class SaCommonHelper
{
	 ...
	 * @since       1.7
	 */
	public static function getFormattedPrice($price, $curr = null, $decimals = 3)
	{
	 ...
	 * @return   array
	 */
	public static function getImpressionAndClicks($adId, $from = null, $to = null, $thirdParty = null)
	{
		$data         = [];
		if ($from || $to || $thirdParty)
		{
			...
			if ($thirdParty)
			{
				$query->where($db3->quoteName('third_party_id') . ' = ' . (int) $thirdParty);
			}
			...
			if ($thirdParty)
			{
				$query->where($db2->quoteName('third_party_id') . ' = ' . (int) $thirdParty);
			}
			...
			$db   = Factory::getDBO();
			$query = $db->getQuery(true);
			$query->select('SUM(aas.impression) as impression');
			$query->select('SUM(aas.click) as click');
			$query->from($db->quoteName('#__ad_archive_stats', 'aas'));
			$query->where($db->qn('ad_id') . ' = ' . (int) $adId);

			if ($from)
			{
				$query->where('Date('.$db2->quoteName('date').')' . ' >= '.  $db2->quote( $from ));
			}

			if ($to)
			{
				$query->where('Date('.$db2->quoteName('date').')' . ' <= '.  $db2->quote( $to ));
			}
			...
			if ($archivestats && count($archivestats))
			{
				$impr += $archivestats[0]->impression;
				$clicks += $archivestats[0]->click;
			}
