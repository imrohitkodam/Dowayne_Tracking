...
class SaAdEngineHelper
{
	...
	public static function getAdsByPriority($params)
	{
		...
		$func_list2 = array();
		if (is_array($params) && isset($params['ad_for_third_party']) && $params['ad_for_third_party'])
		{
			$func_list2 = array('Guest');
		}
		else 
		{
		...
		foreach ($func_list as $func)
		{
			if ($remain != 0 || $remain == '')
			{
				...
				if (!($func == 'Guest' || $func == 'Affiliate' || $func == 'Alt' || $func == 'Map'))
				{
					if ($func != 'Context')
					{
	...
	public static function getQueryWhereCommon($params, $function_name)
	{
		...
		if (self::$_params->get('payment_mode') == 'wallet_mode' && $function_name != "alt" && $function_name != "affiliate" && $function_name != "map")
		{
		...
		// {
		// 	$comon_query[] = " ( NOT EXISTS ( SELECT 1 FROM #__ad_map_target commonamt WHERE commonamt.ad_id = a.ad_id ) ) ";
		// }
	 ...
	 *
	 */
	public static function getAdHtml($addata, $adseen = 0, $adrotation = 0, $widget = '', $thirdPartyId = 0)
	{
		...
		if ($adseen == 1)
		{
			...
			if ($thirdPartyId)
			{
				...
				// $statue_adcharge = self::getad_status($ad->ad_id);
				$adForDisplay = true;
				$statue_adcharge = self::getAdStatus($addata->ad_id, $adForDisplay);
				...
				if ($statue_adcharge['status_ads'] == 1)
				{
					...
					/*self::reduceCredits($addata->ad_id, 0, $statue_adcharge['ad_charge'], $moduleId);*/
					$saCreditsHelper = new SaCreditsHelper;
					$saCreditsHelper->reduceCredits($addata->ad_id, 0, $statue_adcharge['ad_charge'], '', $thirdPartyId);
				...
				$addata->link = Uri::root() . substr(
					Route::_(
						...
						$addata->ad_id . "&caltype=1" . $widget . '&third_party_id=' . $thirdPartyId, false
	 ...
	 * @return  array
	 */
	public static function getAdsForThidParty($thirdPartyId, $zone_id)
	{
		$session = Factory::getSession();
		...
		$params['floater_ad'] = 0;
		$params['num_ads'] = 1;
		$params['ad_rotation'] = 1;
		$params['no_rand'] = 1;
		$params['zone'] = $zone_id;
		$params['alt_ad'] = 1;
		$params['ad_for_third_party'] = 1;
			...
			if (!empty($ad))
			{
				...
				if (empty($ad->seen))
				{
					...
					if ($statue_adcharge['status_ads'] == 1)
					{
						...
						/*self::reduceCredits($ad->ad_id, 0, $statue_adcharge['ad_charge'], $moduleId);*/
						$saCreditsHelper = new SaCreditsHelper;
						$saCreditsHelper->reduceCredits($ad->ad_id, 0, $statue_adcharge['ad_charge'], $moduleId, $thirdPartyId);
