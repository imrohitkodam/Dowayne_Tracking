 ...
 * @since  1.6
 */
class SocialadsModelThirdPartyAdReport extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->join('LEFT', $db->quoteName('#__ad_third_party_enrollment', 'tpe') . 'ON' . $db->quoteName('tpe.id') . '=' . $db->quoteName('as.third_party_id'));
		$query->where($db->quoteName('tpe.created_by') . '=' . $user->id);
	...
	public function getstatsforlinechart()
	{
		...
		if ($ad_id > 0)
		{
			...
			$query1->select('DATE(aas.date) as date, d.ad_id, d.camp_id');
			$query1->select('SUM(aas.impression) as impression');
			$query1->select('SUM(aas.click) as click');
	...
	public function getThirdPartyId()
	{
		...
		$query->from($db->quoteName('#__ad_third_party_enrollment'));
		$query->where($db->quoteName('created_by') . '=' . $user->id);

		$db->setQuery($query);
		$thirdPartyId = $db->loadresult();
	...
	public function thirdPartyAdReportCsvExport()
	{
		...
		foreach ($results as $result)
		{
			...
			$thirdPartyId = $this->getThirdPartyId();
			$impAndCount = SaCommonHelper::getImpressionAndClicks($result->ad_id, $from, $to, $thirdPartyId);
