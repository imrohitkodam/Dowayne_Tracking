...
class SocialadsModelDashboard extends ListModel
{
	...
	public function getActiveAdCount()
	{
		...
		$user  = Factory::getUser();
		$query = $db->getQuery(true);
		$query->select('COUNT(DISTINCT ad.ad_id)')
        ->from($db->quoteName('#__ad_data', 'ad'))
        ->leftJoin($db->quoteName('#__ad_campaign', 'camp') . ' ON ' . $db->quoteName('ad.camp_id') . ' = ' . $db->quoteName('camp.id'))
        ->where($db->quoteName('ad.created_by') . ' = ' . (int) $user->id)
        ->where($db->quoteName('ad.state') . ' = 1')
		->where($db->quoteName('ad.ad_approved') . ' = 1')
        ->where('(' .
            $db->quoteName('ad.ad_noexpiry') . ' = 1' .
            ' OR ' .
            '(' .
                $db->quoteName('ad.ad_noexpiry') . ' = 0' . ' AND ' .
                $db->quoteName('camp.state') . ' = 1' . ' AND ' .
                $db->quoteName('camp.total_budget') . ' >= ' . $db->quoteName('camp.daily_budget') .
            ')' .
        ')');
		$db->setQuery($query);
	...
	/**
	 * Function used to show Inactive ads for a particular user.
	 *
	 * @return  integer
	 *
	 * @since  1.6
	...
	public function getInactiveAdCount()
	{
		...
		$query->select('COUNT(DISTINCT ad.ad_id)')
			->from($db->quoteName('#__ad_data', 'ad'))
			->leftJoin(
				$db->quoteName('#__ad_campaign', 'camp') . ' ON ' .
				$db->quoteName('ad.camp_id') . ' = ' . $db->quoteName('camp.id')
			)
			->where($db->quoteName('ad.created_by') . ' = ' . (int) $user->id)
			->where('(' .
				$db->quoteName('ad.state') . ' = 0' .
				' OR (' .
					$db->quoteName('ad.state') . ' = 1' . ' AND ' .
					$db->quoteName('ad.ad_noexpiry') . ' = 0' . ' AND (' .
						$db->quoteName('ad.camp_id') . ' = 0' .
						' OR (' .
								$db->quoteName('camp.state') . ' = 1' . ' AND (' .
								$db->quoteName('camp.total_budget') . ' < ' . $db->quoteName('camp.daily_budget') .
								' OR ' . $db->quoteName('ad.ad_approved') . ' != 1' .
							')' .
						')' .
					')' .
				')' .
				' OR (' .
					$db->quoteName('ad.state') . ' = 1' . ' AND ' .
					$db->quoteName('ad.ad_noexpiry') . ' = 1' . ' AND ' .
					$db->quoteName('ad.ad_approved') . ' != 1' .
				')' .
			')');
