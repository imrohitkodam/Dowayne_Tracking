...
class SocialadsModelAdForm extends AdminModel
{
	...
	public function getZonesData($typ)
	{
		...
		foreach ($zone_data as $zone)
		{
			...
			$z[]     = array(
				...
				"layout"    => $layouts,
				"use_image_ratio"    => $zone->use_image_ratio,
				"img_width_ratio"    => $zone->img_width_ratio,
				"img_height_ratio"   => $zone->img_height_ratio,
				"is_responsive"      => $zone->is_responsive,
				"base"      => Uri::Root()
			);
		}
	...
	public function saveDesignAd($formData, $adminApproval, $isCopyAd = false)
	{
		...
		$designAd_data->ad_payment_type = $formData['pricing_opt'];
	...
	public function saveTargetingData($formData, $isCopyAd = false)
	{
		...
		$ad_id       = $session->get('ad_id');
		$geotargetId = $adfield_id = $context_targetId = $maptargetId = '';
		...
		if ($ad_id)
		{
			...
			$maptargetAdId = $this->getColumnFromAnyFieldValue($adData->ad_id, 'ad_id', '#__ad_map_target');

			if ($maptargetAdId)
			{
				$maptargetId = $maptargetAdId;
			}
		}
		else
		{
		...
		{
			// Added for map targeting
			$map_target     = isset ($formData['map_targett']) ? $formData['map_targett'] : 0;
			$enterlocation     = isset ($formData['selectlocation']) ? $formData['selectlocation'] : 0;
			$selectlocationonmap     = isset ($formData['selectlocationonmap']) ? $formData['selectlocationonmap'] : 0;
			$map_targett_id     = isset ($formData['map_targett_id']) ? $formData['map_targett_id'] : 0;
			$mapData = $formData['map'];
			$mapDetails = isset ($mapData['details']) ? json_decode($mapData['details']) : '';
			$mapPopulationCount = isset ($mapData['population']) ? $mapData['population'] : '';

			$locationData = $formData['location'];
			$locationCount = isset ($locationData['count']) ? $locationData['count'] : '';
			$input = $app->input;
			$idMap = $input->get('location_id_map', [], 'array');
			...
			if ($map_target == 1)
			{
				...
				{
					// Fetch third_party_ids from request
					$app   = JFactory::getApplication();
					$input = $app->input;

					$mapData = $input->get('map', array(), 'ARRAY');
					$thirdPartyIds = isset($mapData['third_party_ids']) ? $mapData['third_party_ids'] : [];

					// Make sure it's always an array
					if (!is_array($thirdPartyIds)) {
						// Agar string hai "19,25" to array me convert kar do
						$thirdPartyIds = explode(',', $thirdPartyIds);
					}
					...
					$fielddata->id      = '';
					$fielddata->ad_id   = $ad_id;
					$fielddata->count   = $mapPopulationCount;
					$fielddata->location_on = 0;

					if ($maptargetId)
					{
						$fielddata->id = $maptargetId;
					}

					if ($fielddata->id)
					{
						if (!$this->_db->updateObject('#__ad_map_target', $fielddata, 'id'))
						{
							echo $this->_db->stderr();
							return false;
						}
					}
					elseif (!$this->_db->insertObject('#__ad_map_target', $fielddata, 'id'))
					{
						echo $this->_db->stderr();
						return false;
					}
					...
					if (!$maptargetId)
					{
						$maptargetId = $this->_db->insertid();
					}

					// Fetch already stored locations
					$query = $this->_db->getQuery(true);
					$query->select($this->_db->quoteName('location'))
						->from($this->_db->quoteName('#__ad_map_target_locations'))
						->where($this->_db->quoteName('ad_map_target_id') . ' = ' . (int) $maptargetId);
					$this->_db->setQuery($query);
					$presentLocations = $this->_db->loadColumn();

					if ($mapDetails && count($mapDetails))
					{
						$newLocations = [];

						foreach ($mapDetails as $details)
						{
							$array = (array) $details;
							$newLocations[] = '(' .$array['lat'] . ', '. $array['lng'] .')';
						}

						foreach($mapDetails as $locationDetail)
						{
							$array = (array) $locationDetail;
							if (!in_array(('(' .$array['lat'] . ', '. $array['lng'] .')'), $presentLocations))
							{
								// Loop through each third_party_id and insert separate record
								foreach ($thirdPartyIds as $id)
								{
									$fielddata                     = new stdClass;
									$fielddata->id                 = '';
									$fielddata->ad_map_target_id   = $maptargetId;
									$fielddata->location           = '(' .$array['lat'] . ', '. $array['lng'] .')';
									$fielddata->radius             = $array['radius'];
									$fielddata->city               = $array['city'];
									$fielddata->region             = $array['region'];
									$fielddata->country            = $array['country'];
									$fielddata->population         = $array['population'];
									$fielddata->third_party_id     = (int) trim($id); // ✅ each ID inserted in new row

									if (!$this->_db->insertObject('#__ad_map_target_locations', $fielddata, 'id'))
									{
										echo $this->_db->stderr();
										return false;
									}
						...
						$deletedLocations = array_diff($presentLocations, $newLocations);

						if ($deletedLocations)
						{
							$query = $this->_db->getQuery(true);
							$conditions = array(
								$this->_db->quoteName('location') . ' IN (' . implode(',', $this->_db->quote($deletedLocations)) . ')'
							);

							$query->delete($this->_db->quoteName('#__ad_map_target_locations'))
								->where($conditions);
					...
					{
						$query = "DELETE FROM #__ad_map_target_locations WHERE ad_map_target_id=" . (int) $maptargetId;
						$this->_db->setQuery($query);
				...
				// 					$fielddata->country            = $array['country'];
				// 					$fielddata->population         = $array['population'];
				// 					$fielddata->third_party_id     = (int) $id; // ✅ each ID inserted in new row
				...
				else if ($enterlocation)
				{
					$newLocations = isset($locationData['location']) ? $locationData['location'] : array();

					$type               = str_replace("by", "", $geo_type);
					$fielddata          = new stdClass;
					$fielddata->id      = '';
					$fielddata->ad_id   = $ad_id;
					// $fielddata->location  = $newLocations;
					$fielddata->count  = $locationCount;
					$fielddata->location_on = 1;
					...
					$query	= $this->_db->getQuery(true);
					$query->select($this->_db->quoteName('location'));
					$query->from($this->_db->quoteName('#__ad_map_target_locations'));
					$query->where($this->_db->quoteName('ad_map_target_id') . ' = ' . $maptargetId);
					...
					if ($newLocations)
					{
						foreach($newLocations as $locationDetail)
						{
							if (!in_array($locationDetail, $presentLocations))
							{
								$fielddata          = new stdClass;
								$fielddata->id      = '';
								$fielddata->ad_map_target_id   = $maptargetId;
								$fielddata->location  = $locationDetail;
								// $fielddata->location_on = 1;
								$fielddata->third_party_id = $idMap[$locationDetail] ?? null;
					...
					{
						$query = "DELETE FROM #__ad_map_target_locations WHERE ad_map_target_id=" . $maptargetId;
						$this->_db->setQuery($query);
				...
				{
					$query = "DELETE FROM #__ad_map_target WHERE ad_id=" . $ad_id;
					$this->_db->setQuery($query);
			...
			else
			{
				...
				// If ,ap targeting is disabled then delete all map locations
				if ($maptargetId)
				{
					$query = "DELETE FROM #__ad_map_target_locations WHERE ad_map_target_id=" . $maptargetId;
					$this->_db->setQuery($query);
	...
	public function savePricingData($formData, $isCopyAd = false)
	{
		...
		if ($sa_params->get('payment_mode') == 'wallet_mode')
		{
			...
			$camp_total_budget = $formData['camp_total_budget'];
			$camp_start_date = $formData['camp_start_date'];
			$camp_end_date = $formData['camp_end_date'];
			if (!$camp_id && !empty($camp_name) && !empty($camp_amount))
			{
				...
				$obj->total_budget = isset($formData['camp_total_budget']) && $formData['camp_total_budget'] !== '' ? $formData['camp_total_budget'] : 0;
				$obj->start_date = $formData['camp_start_date'];
				$obj->end_date = $formData['camp_end_date'];
				$obj->state        = 1;
	 ...
	 * @since  3.0
	 **/
	public function getData_map_target($ad_id)
	{
		$db    = Factory::getDbo();
		$user  = Factory::getUser();
		$input = Factory::getApplication()->input;
		...
		$query	= $db->getQuery(true);
		$query->select('a.*');
		$query->from($db->quoteName('#__ad_map_target', 'a'));
		$query->where($db->quoteName('a.ad_id') . ' = ' . $ad_id);
		$db->setQuery($query);
		$addata = $db->loadAssocList();
		...
		if (!empty($addata[0]))
		{
			if ($addata[0]['location_on'] == 1)
			{
				$query	= $db->getQuery(true);
				$query->select($db->quoteName('location'));
				$query->from($db->quoteName('#__ad_map_target_locations', 'a'));
				$query->where($db->quoteName('a.ad_map_target_id') . ' = ' . $addata[0]['id']);
				$db->setQuery($query);
				$locations = $db->loadColumn();
				$addata[0]['locations'] = $locations;
			...
			{
				$query	= $db->getQuery(true);
				$query->select($db->quoteName(array('location', 'radius', 'city', 'region', 'country', 'population')));
				$query->from($db->quoteName('#__ad_map_target_locations', 'a'));
				$query->where($db->quoteName('a.ad_map_target_id') . ' = ' . $addata[0]['id']);
				$db->setQuery($query);
				...
				{
					$address = str_replace('(', '', $location->location);
					$address = str_replace(')', '', $address);
					$address = explode(', ', $address);
					$locations[$key]->lat = $address[0];
					$locations[$key]->lng = $address[1];
				}
	...
	/**
	 * Retrieve details for a third party locations
	 *
	 * @return  object  $locations  Details
	 *
	 * @since   4.2.1
	 */
	// public function getThirdPartyLocations()
	// {

	// 	$thirdPartylocations = array();
	// 	$db = Factory::getDbo();
	// 	$query	= $db->getQuery(true);
	// 	$query->select(array('l.city', 'l.region', 'l.country'))
	// 		->select('SUM(l.count) as location_count')
	...
	// 		->order($db->quoteName('city'))
	// 		->group($db->quoteName('city'));

	// 	$db->setQuery($query);
	// 	$thirdPartyLocationList = $db->loadObjectList();

	// 	foreach ($thirdPartyLocationList as $thirdPartyLocation)
	// 	{
	// 		if ($thirdPartyLocation->location_count != 0)
	// 		{
	// 			$value = $thirdPartyLocation->city . ', ' . $thirdPartyLocation->region . ', ' . 
	// 				$thirdPartyLocation->country . ' (' . $thirdPartyLocation->location_count . ')';
	// 			array_push($thirdPartylocations, $value);
	// 		}
	// 	}
		
	// 	// Get trasalated string.
	// 	return $thirdPartylocations;
	// }
	...
	public function getThirdPartyLocations()
	{
		...
		$db = Factory::getDbo();
		$query	= $db->getQuery(true);
		$query->select(array('l.city', 'l.region', 'l.country','l.third_party_id'))
			->select('SUM(l.count) as location_count')
			->from($db->quoteName('#__ad_third_party_enrollment', 'a'))
			->join('LEFT', $db->quoteName('#__ad_third_party_locations', 'l') . 'ON' . $db->quoteName('l.third_party_id') . '=' . $db->quoteName('a.id'))
			->where($db->quoteName('a.state') . ' = 1' )
		...
		foreach ($thirdPartyLocationList as $thirdPartyLocation)
		{
			...
			if ($thirdPartyLocation->location_count !== 0)
			{
				$value = $thirdPartyLocation->city . ', ' . $thirdPartyLocation->region . ', ' . $thirdPartyLocation->country . ' (' . $thirdPartyLocation->location_count . ')';
			
				$thirdPartylocations[] =  [
					'id'    => $thirdPartyLocation->third_party_id,
					'value' => $value
				];
