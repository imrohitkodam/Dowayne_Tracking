...
var sa = {
	...
	create: {
					...
					if (data.direction === 'next') {
						...
						techjoomla.jQuery.ajax({
							...
							success: function (response) {
								...
								{
									if (typeof response['mapTargetData'] === 'object' && response['mapTargetData'] && response['mapTargetData'].count !== undefined)
									{
										jQuery('.totalPopulationInfo').removeClass('d-none');
										jQuery('.totalPopulationInfo .totalPopulationCount').text(response['mapTargetData'].count);
									}
									else
									{
										jQuery('.totalPopulationInfo').addClass('d-none');
									}
								}
		...
		getStatesList: function (countryId, Dbvalue, selOptionMsg, id = "state") {
			...
			var callFor = '';
			if (jQuery('.selectLocationOnMapDiv').length)
			{
				callFor = '&callfor=getPopulation'
			}
			techjoomla.jQuery.ajax({
				url: Joomla.getOptions('system.paths').base + '/index.php?option=com_sa&task=checkout.loadState&country=' + country + '&tmpl=component' + callFor,
				type: 'GET',
				dataType: 'json',
				success: function (data) {
					...
					sa.create.generateStatesOptions(data, countryId, Dbvalue, selOptionMsg, id);
				}
			});
		},

		generateStatesOptions: function (data, countryId, Dbvalue, selOptionMsg, id = "state") {
			var country = techjoomla.jQuery('#' + countryId).val();
		...
		getZonePricing: function () {
			...
			if (val == 1) {
				...
				var combinedCost = parseFloat(click_price) + parseFloat(click_imp);

				// Agar HTML me koi dedicated span nahi hai to create dynamically
				if (!document.getElementById('combined_span')) {
					console.log('Creating combined_span element dynamically...');
					var combinedEl = document.createElement('div');
					combinedEl.id = 'combined_span';
					combinedEl.style.fontWeight = 'bold';
					combinedEl.style.marginTop = '5px';
					bid_val.appendChild(combinedEl);
				}

				jQuery('#combined_span').html(
					Joomla.JText._('COM_SOCIALADS_COMBINED_COST') + getFormattedPrice(combinedCost, currency)
				);
			}
		},
...
var saAdmin = {
	...
	campaign: {
		...
		initCampaignJs: function () {
			...
			techjoomla.jQuery(document).ready(function () {
				...
				var campaigndailyBudget, campaigntotalBudget, campaignStartDate, campaignEndDate, campaignNoOfdays, availableBalance;

				// Function to calculate the total budget based on daily budget and campaign duration
				function calculateTotalBudget() {
					const dailyBudget = parseFloat(jQuery('#jform_daily_budget').val());
					const startDate = new Date(jQuery('#jform_start_date').val());
					const endDate = new Date(jQuery('#jform_end_date').val());
	
					if (dailyBudget && startDate && endDate && endDate >= startDate) {
						const days = (endDate - startDate) / (1000 * 60 * 60 * 24) + 1;
						jQuery('#jform_total_budget').val((dailyBudget * days).toFixed(2));
					}
				}

				// Function to calculate the daily budget based on total budget and campaign duration
				function calculateDailyBudget() {
					const totalBudget = parseFloat(jQuery('#jform_total_budget').val());
					const startDate = new Date(jQuery('#jform_start_date').val());
					const endDate = new Date(jQuery('#jform_end_date').val());
	
					if (totalBudget && startDate && endDate && endDate >= startDate) {
						const days = (endDate - startDate) / (1000 * 60 * 60 * 24) + 1;
						jQuery('#jform_daily_budget').val((totalBudget / days).toFixed(2));
					}
				}

				// Function to calculate the start date and end date based on daily budget and total budget of campaign
				function calculateStartDateAndEndDate() {
					// Normalize today's date to YYYY-MM-DD (local)
					const now = new Date();
					now.setHours(0,0,0,0);
					const todayStr = new Date(now.getTime() - now.getTimezoneOffset() * 60000).toISOString().split('T')[0];
					const today = new Date(todayStr);
					
					const startInput = jQuery('#jform_start_date');
					const endInput = jQuery('#jform_end_date');
					const dailyBudget = parseFloat(jQuery('#jform_daily_budget').val());
					const totalBudget = parseFloat(jQuery('#jform_total_budget').val());
					
					// If budgets are valid, compute dates from budgets using today as start
					if (dailyBudget && totalBudget && dailyBudget > 0) {
						const daysToAdd = Math.ceil(totalBudget / dailyBudget) - 1;
						const endDateObj = new Date(today);
						endDateObj.setDate(endDateObj.getDate() + daysToAdd);
						
						const startDateStr = todayStr;
						const endDateStr = new Date(endDateObj.getTime() - endDateObj.getTimezoneOffset() * 60000).toISOString().split('T')[0];
						
						startInput.val(startDateStr);
						endInput.val(endDateStr);
					} else {
						// If budgets are not usable, still enforce min dates if user-entered
					...
					// Ensure end >= start
					const sVal = startInput.val();
					const eVal = endInput.val();
					if (sVal && eVal) {
						...
						e.setHours(0,0,0,0);
						if (e < s) {
							// If end before start, set end = start
							endInput.val(sVal);
						}
					}
					
					// Set min attributes
					startInput.attr('min', todayStr);
					endInput.attr('min', startInput.val() || todayStr);
				}

				// Event handlers to recalculate budget values on input change
				jQuery('#jform_daily_budget, #jform_start_date, #jform_end_date').on('input change', function () {
					calculateTotalBudget();
				});
	
				jQuery('#jform_total_budget').on('blur', function () {
					calculateDailyBudget();
					calculateStartDateAndEndDate();
				});
	
				// Event handler to ensure date validation and recalculation on date change
				jQuery('#jform_start_date, #jform_end_date').on('change', function () {
					// Use local date for min attribute to avoid UTC shift
					var _now = new Date();
					var _today = _now.getFullYear() + '-' + String(_now.getMonth() + 1).padStart(2, '0') + '-' + String(_now.getDate()).padStart(2, '0');
					jQuery('#jform_start_date').attr('min', _today);
					jQuery('#jform_end_date').attr('min', jQuery('#jform_start_date').val());
					calculateTotalBudget();
				});

				// Ensure form fields are updated before form submission (site and admin)
				var $form = jQuery('#campaign-form');
				if (!$form.length) { $form = jQuery('#adminForm'); }
				$form.on('submit', function() {
					// If dates are empty but budgets exist, auto-calculate before submit
					var startDate = jQuery('#jform_start_date').val();
					var endDate = jQuery('#jform_end_date').val();
					var dailyBudget = parseFloat(jQuery('#jform_daily_budget').val());
					var totalBudget = parseFloat(jQuery('#jform_total_budget').val());
					if ((!startDate || !endDate) && dailyBudget && totalBudget) {
						calculateStartDateAndEndDate();
						startDate = jQuery('#jform_start_date').val();
						endDate = jQuery('#jform_end_date').val();
					}

					// Validate using local YYYY-MM-DD string comparison
					var __now = new Date();
					var todayStr = __now.getFullYear() + '-' + String(__now.getMonth() + 1).padStart(2, '0') + '-' + String(__now.getDate()).padStart(2, '0');

					// Start must be today or future
					if (startDate && startDate < todayStr) {
						alert('Start date cannot be in the past.');
						return false;
					}

					// End must be blank or >= start
					...
					// Debug logging
					console.log('Campaign form submit handler - Date values:', {
						startDate: startDate,
						endDate: endDate
					});
					return true;
		...
		campaignSubmitButton: function (task) {
			if (task == 'campaign.cancel') {
				...
				var startDateInput = jQuery('#jform_start_date');
				var endDateInput = jQuery('#jform_end_date');
				var startDate = startDateInput.val();
				var endDate = endDateInput.val();
				
				// Validate using local YYYY-MM-DD string comparison
				var _now2 = new Date();
				var todayStr2 = _now2.getFullYear() + '-' + String(_now2.getMonth() + 1).padStart(2, '0') + '-' + String(_now2.getDate()).padStart(2, '0');

				// Start must be today or future
				if (startDate && startDate < todayStr2) {
					alert('Start date cannot be in the past.');
					return false;
				}
				...
				console.log('Campaign form submission - Formatted dates:', {
					startDate: startDate,
					endDate: endDate
				});
	
				// Final guard: compare dates again
	...
	zone: {
		zoneAdTypes: function (value) {
			...
			if (!value) {
				...
				techjoomla.jQuery('#img_width_ratio').hide();
				techjoomla.jQuery('#img_height_ratio').hide();
				techjoomla.jQuery('#layout_row').hide();
			}
		},
		trim: function (str) {
	...
	thirdpartyenrollments: {
		submitButtonAction: function (action) {
			if (action == 'thirdpartyenrollments.publish' || action == 'thirdpartyenrollments.unpublish') {
				Joomla.submitform(action);
			} else if (action == 'thirdpartyenrollments.delete') {
				var r = confirm(Joomla.JText._('COM_SOCIALADS_THIRD_PARTY_ENROLLMENTS_DELETE_CONFIRM'));
				if (r != true) {
					return;
				}
			} else {
				window.location = 'index.php?option=com_socialads&view=thirdpartyenrollments';
			}
