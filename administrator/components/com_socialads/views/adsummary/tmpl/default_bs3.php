...
<div class="<?php echo SA_WRAPPER_CLASS; ?> tj-adsummary" id="sa-adsummary">
					...
					if ($this->callFromAdReport)
					{
						...
						<div class="span6">
							<div class="panel panel-default">
								<div class="panel-heading">
									<i class="fa fa-bar-chart-o fa-fw"></i>
										<b><?php echo Text::_('COM_SOCIALADS_BAR_CHART_STAT'); ?></b>
								</div>
								<div class="panel-body">
									<div id="graph-monthly-sales-third-party-report">
									</div>
									<div class="center" id="graph-monthly-sales-third-party-report-msg">
	...
	<input type="hidden" id="adIdForBarChart" value="<?php echo $this->adid; ?>">
	<input type="hidden" id="thirdPartyIdForBarChart" value="<?php echo $this->thirdPartyId; ?>">
</div>
<script>
	...
	techjoomla.jQuery(document).ready(function() {
		...
		var data = {
			...
			to : jQuery('#to').val(),
			adid : jQuery('#adIdForBarChart').val(),
			thirdpartyid : jQuery('#thirdPartyIdForBarChart').val(),
		}

		techjoomla.jQuery.ajax({
			type: 'GET',
			url: 'index.php?option=com_socialads&task=thirdpartyadreports.getBarChartData',
			data: data,
			async: true,
			dataType: 'json',
			success: function (response) {
				/*Hide no data msg*/
				techjoomla.jQuery('#graph-monthly-sales-third-party-report-msg').hide();
				/*Draw Chart*/
				console.log(response)
				Morris.Bar({
					element: 'graph-monthly-sales-third-party-report',
					data: response,
					xkey: 'month',
					ykeys: ['clicks', 'impressions'],
					labels: 'Clicks and Impressions',
					barColors: ['#FFA500','#3EA99F'],
					barRatio: 0.4,
					xLabelAngle: 35,
					hideHover: 'auto',
					resize: true
