			...
			<form action="<?php echo Route::_('index.php?option=com_socialads&view=adorders'); ?>" method="post" name="adminForm" id="adminForm">
				<div id="filter-bar" class="btn-toolbar">
					...
					if (JVERSION >= '3.0') : ?>
						...
						<div class="btn-group pull-right hidden-phone">
							<?php echo HTMLHelper::_('select.genericlist', $this->gatewayoptions, "filter_gatewaylist", 'class="ad-status inputbox input-medium" size="1" onchange="document.adminForm.submit();" name="gatewaylist"', "value", "text", $this->state->get('filter.gatewaylist')); ?>
						</div>
					<?php
					endif; ?>
				</div>
							...
							<tr class="row<?php echo $i % 2; ?> <?php echo $row_color; ?>">
								...
								<td class="tj-width-10" data-title="<?php echo Text::_('COM_SOCIALADS_ORDERS_STATUS');?>">
									<?php
										...
										{
											echo HTMLHelper::_('select.genericlist',$validStatus,'pstatus'.$k,'class="pad_status"  onChange="saAdmin.orders.selectStatusOrder(' . $item->id . ',this);"',"value","text",$item->status);
										}
										else
										{
											echo $whichever ;
