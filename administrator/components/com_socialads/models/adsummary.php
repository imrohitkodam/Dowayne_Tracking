...
class SocialadsModelAdsummary extends ListModel
{
	...
	public function getstatsforpiechart()
	{
		...
		if ($thirdPartyId)
		{
			$query->where($db->qn('s.third_party_id') . ' = ' . $thirdPartyId);
		}
		...
		if ($thirdPartyId)
		{
			$query1->where($db->qn('aas.third_party_id') . ' = ' . $thirdPartyId);
		}
		...
		if ($thirdPartyId)
		{
			$query2->where($db->qn('s.third_party_id') . ' = ' . $thirdPartyId);
		}
		...
		if ($thirdPartyId)
		{
			$query3->where($db->qn('aas.third_party_id') . ' = ' . $thirdPartyId);
		}
