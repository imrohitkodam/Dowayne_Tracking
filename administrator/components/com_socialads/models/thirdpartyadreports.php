 ...
 * @since  1.6
 */
class SocialadsModelThirdPartyAdReports extends ListModel
{
	/**
	 * Constructor.
	 *
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$thirdParty = $app->getUserStateFromRequest($this->context . '.filter.thirdpartylist', 'filter_thirdpartylist', '', 'string');
		$this->setState('filter.thirdpartylist', $thirdParty);
	...
	protected function getListQuery()
	{
		...
		$query->join('LEFT', $db->quoteName('#__ad_third_party_enrollment', 'tpe') . 'ON' . $db->quoteName('tpe.id') . '=' . $db->quoteName('as.third_party_id')); // IDL
		...
		$thirdParty = $this->getState('filter.thirdpartylist');

		if ($thirdParty)
		{
			$query->where($db->quoteName('as.third_party_id'). ' = ' . (int) $thirdParty);
		}
	...
	public function thirdPartyAdReportCsvExport()
	{
		...
		$filter_search = $_POST['filter_search'] ? $_POST['filter_search'] : null;
		$from = $_POST['from'] ? $_POST['from'] : null;
		$to = $_POST['to'] ? $_POST['to'] : null;
		$thirdParty = $_POST['filter_thirdpartylist'] ? $_POST['filter_thirdpartylist'] : null;
		$filter_zonelist = $_POST['filter_zonelist'] ? $_POST['filter_zonelist'] : null;
		$filter_published = isset($_POST['filter_published']) ? $_POST['filter_published'] : null;

		$this->setState('filter.search', $filter_search);
		$this->setState('filter.state', $filter_published);
		$this->setState('filter.from', $from);
		$this->setState('filter.to', $to);
		$this->setState('filter.zonelist', $filter_zonelist);
		$this->setState('filter.thirdpartylist', $thirdParty);
		...
		foreach ($results as $result)
		{
			...
			if ($from || $to || $thirdParty)
			{
				require_once JPATH_SITE . "/components/com_socialads/helpers/common.php";

				$impAndCount = SaCommonHelper::getImpressionAndClicks($result->ad_id, $from, $to, $thirdParty);
	...
	public function monthlyChartData()
	{
		...
		$thirdPartyId = $input->get('thirdpartyid', 0, 'INT');
		$from       = $input->get('from');
		$to         = $input->get('to');
		$statistics = array();
		...
		$query->select('MONTH( time ) AS monthsname, COUNT(IF(display_type="1",1, NULL)) as click , COUNT(IF(display_type="0",1, NULL)) as impression');
		$query->from($db->quoteName('#__ad_stats'));
		$query->where($db->quoteName('ad_id') . '=' . $ad_id);

		if ($thirdPartyId)
		{
			$query->where($db->qn('third_party_id') . ' = ' . $thirdPartyId);
		}

		$query->where("DATE(time) BETWEEN DATE('" . $from_date . "') AND DATE('" . $to_date . "')");
		$query->group('MONTH( time )');
		$query->order('MONTH( time )');
		$db->setQuery($query);
		$stats = $db->loadObjectlist();

		// Query to get data from archive stats
		$query1->select('MONTH( date ) AS monthsname');
		$query1->select('SUM(impression) as impression');
		$query1->select('SUM(click) as click');
		$query1->from($db->quoteName('#__ad_archive_stats'));
		$query1->where($db->quoteName('ad_id') . '=' . $ad_id);

		if ($thirdPartyId)
		{
			$query1->where($db->qn('third_party_id') . ' = ' . $thirdPartyId);
		}

		$query1->where("DATE(date) BETWEEN DATE('" . $from_date . "') AND DATE('" . $to_date . "')");
		$query1->group('MONTH( date )');
		$query1->order('MONTH( date )');
		$db->setQuery($query1);
