...
// error_log('ThirdParty controller loaded');
...
class SocialadsControllerThirdParty extends SocialadsController
{
	...
	public function save()
	{
		...
		if (empty($populationAreas))
		{
			$app->setUserState('com_socialads.edit.thirdparty.data', $data);
			$app->enqueueMessage(Text::_('COM_SOCIALADS_ERROR_SELECT_SERVICE_AREA'), 'error');
			$this->setRedirect('index.php?option=com_socialads&view=thirdparty&layout=edit' . ($id ? ('&id=' . $id) : ''));
        	return false;
		}
		...
		// Set created_by field for new records
		if (!$id) {
		...
		$url = 'index.php?option=com_socialads&view=thirdpartyenrollments';
	...
	public function edit()
	{
		...
		$id  = isset($cid[0]) ? (int) $cid[0] : 0;
		
		if ($id)
		{
			$this->setRedirect('index.php?option=com_socialads&view=thirdparty&layout=edit&id=' . $id);
		}
	}
	...
	public function add()
	{
		$app   = Factory::getApplication();
		$app->setUserState('com_socialads.edit.thirdparty.data', null);
		$this->setRedirect('index.php?option=com_socialads&view=thirdparty&layout=edit');
	}
	...
	public function cancel()
	{
		$app   = Factory::getApplication();
		$app->setUserState('com_socialads.edit.thirdparty.data', null);
		$this->setRedirect('index.php?option=com_socialads&view=thirdpartyenrollments');
	}
